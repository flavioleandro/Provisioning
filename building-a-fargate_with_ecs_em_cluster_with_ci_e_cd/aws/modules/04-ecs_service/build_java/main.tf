...
resource "aws_codebuild_project" "codebuild_java" {
  name         = "${var.env}-${var.project_name}-${var.service_name}-java"
  description  = "Build the jar file and send it as output_artifact"
  ...
  source { 
    type      = "S3"
    location  = "claro-artifact-store-${var.project_name}/${var.env}-${var.service_name}.zip"
    buildspec = "buildspec_java.yml"
  }
}
...
resource "aws_codepipeline" "codepipeline" {
  ...
  stage {
    ...
    action {
      ...
      output_artifacts = ["build_jar_artifact"]

      configuration = {
        ProjectName = "${aws_codebuild_project.codebuild_java.name}"
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      provider         = "CodeBuild"
      version          = "1"
      input_artifacts  = ["build_jar_artifact"]
      output_artifacts = ["build_docker_artifact"]
  ...
  stage {
    ...
    action {
      ...
      provider        = "ECS"
      version         = "1"
      input_artifacts = ["build_docker_artifact"]
